%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Welfare_output, UE_output, MEC_output, Bid_output, Price_output] = Dynamic_Case_GSP_MEC(T,Nrun,I_count,R,J,K_limit,d_min,d_max)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Nrun = 1;                   % no. of iterations to average the results
N = 1;                      % no. of MEC application/processor at the orchestrator
I = I_count;                      % no. of MEC servers/nodes 
K = K_limit;
...
C = 4e6*[32; 24; 24; 16; 16];               % computing power (MIPS) --> convert to MB/sec (where 1 instruction is stored as 4Byte)
%------------------------------------------------------------------
...
% rho=[0.0223; 0.0208; 0.0211; 0.0186; 0.0175];   % scaling parameter for $/VM-hr --> $/VM-sec rates
rho=[0.0452; 0.0435; 0.0385; 0.0186; 0.0175];
...
%--------------------------------------------------------------------------
d_avg = randi([d_min,d_max],1,J);       % average data size between d_min and d_max
...
%             d_k(j_s,n,t,nr) = poissrnd(d_avg(j_s))*1e6;     % data size (MB) follow Poisson distribution
        ...
        [x_RBB(:,:,:,t,nr),eta_RBB(:,:,t,nr),p_RBB(:,:,t,nr),upm_RBB(:,:,t,nr),u_MEC_RBB(:,t,nr),z_RBB(:,t,nr),eta_rem_RBB(:,:,t,nr)] = GSP_Mechanism(R,theta_RBB(:,:,t,nr),eta_RBB_prev,b_RBB(:,:,t,nr));
        [x_BB(:,:,:,t,nr),eta_BB(:,:,t,nr),p_BB(:,:,t,nr),upm_BB(:,:,t,nr),u_MEC_BB(:,t,nr),z_BB(:,t,nr),eta_rem_BB(:,:,t,nr)] = GSP_Mechanism(R,theta_BB(:,:,t,nr),eta_BB_prev,b_BB(:,:,t,nr));
        [x_AB(:,:,:,t,nr),eta_AB(:,:,t,nr),p_AB(:,:,t,nr),upm_AB(:,:,t,nr),u_MEC_AB(:,t,nr),z_AB(:,t,nr),eta_rem_AB(:,:,t,nr)] = GSP_Mechanism(R,theta_AB(:,:,t,nr),eta_AB_prev,b_AB(:,:,t,nr));
        [x_CB(:,:,:,t,nr),eta_CB(:,:,t,nr),p_CB(:,:,t,nr),upm_CB(:,:,t,nr),u_MEC_CB(:,t,nr),z_CB(:,t,nr),eta_rem_CB(:,:,t,nr)] = GSP_Mechanism(R,theta_CB(:,:,t,nr),eta_CB_prev,b_CB(:,:,t,nr));
...
Bid_output = [bMEC_avg_RBB; bMEC_avg_BB; bMEC_avg_AB; bMEC_avg_CB; bMEC_avg_VCG];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x,eta,p,upm_MEC,u_MEC_RBB,z_RBB,eta_rem_RBB] = GSP_Mechanism(R,theta,eta_prev,b)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%---------------------------------------------------
f = 5.8;            % transmission frequency IEEE 802.11ah-->1GHz and 802.11ac--> 5.8GHz
BW = 80*1e6;            % channel bandwidth for IEEE 802.11ah--> 2MHz and 802.11ac--> 80MHz
% L_n = 24;                                       % power loss coefficient in indoor office space according to ITU recommendation
% L_f = 17;                                       % indoor shadow fading,  (17 dB --> 10.^1.7)


P_u = 0.1;                                       % User's transmit power (in watt) 10dBm(IoT device) --> 0.1 watt, 20 dBm(smart device)
...
for j = 1:J
    ...
    i = MEC(j);
%     s = randi([1,2]);                                   % randomly chooses user type (1)IoT device(802.11ah) or (2)smart device(802.11ac)
